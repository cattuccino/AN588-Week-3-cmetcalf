...
summary(d)
```
This summary of all the data tells you the median area=69,700 and the median population size=4.912e+06 for all the countries since it gives a summary of the overall.
...
```{r}
 tail (d, 20)
```
This gives the largest countries (since they are at the end of the order), d designates it as the whol data frame where d$density would just list the densities of the largest countries.
...
```{r}
p <- ggplot(data = d, aes(x = log(Body_mass_female), y = log(Body_mass_male), z = log(Mass_Dimorphism)),
+     color = factor(Family)))  # first, we build a plot object and color points by Family
> p <- p + xlab("log(Female Body Mass)") + ylab("log(Male Body Mass)") + zlab("log(Sexual Dimorphism)"))  # then we modify the axis labels
> p <- p + geom_point()  # then we make a scatterplot
> p <- p + theme(legend.position = "bottom", legend.title = element_blank())  # then we modify the legend
> p  # and, finally, we plot the object
```
*error in above chunk*
***
...
pop_sd(x)
```
*does not match output from MODULE!
```{r}
...
ci
```
*error given, cant find function -> have to install {sciplot} package? Did not fix it, still can not find function -> type it with library() -> did not give correct outputs
```{r}
